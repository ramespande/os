...
static void proc_yield() {
    ...
    if (curr_proc_idx != 0 && curr_proc_idx <= MAX_NPROCESS) {
        ...
        if (curr->status != PROC_UNUSED && curr->last_scheduled_time > 0) {
            ulonglong runtime = current_time - curr->last_scheduled_time;
            curr->total_cpu_time += runtime;
            
            mlfq_update_level(curr, runtime);
        }
    }
    ...
    if (next_idx < MAX_NPROCESS) {
        ...
        if (next->first_run) {
            next->first_scheduled_time = schedule_time;
            next->first_run = 0;
        }
        
        next->last_scheduled_time = schedule_time;
